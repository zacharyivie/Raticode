...
These private options are version-sensitive. Resource startup checks `--version` for the inspected `2026.09.10` and `2026.09.15` build families and checks `--help` for Cursor identity and `--plugin-dir`. Other versions fail explicitly with the detected version, supported families, and instructions to update Raticode or select a supported executable. A successful `--help` call alone is insufficient evidence for the hidden resource flags. Normal authenticated Cursor mode supports user-local plugins; local/Bedrock mode does not, and the documented local-mode environment switches are rejected. The private config does not relocate the credential backend: Linux credentials use XDG configuration, macOS uses the home directory/keychain. Raticode does not read or copy those credentials. Existing account-specific soft CLI settings are not copied into the private config.
